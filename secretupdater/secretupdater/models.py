...
def process(event):
    ...
    for service in affected_services:
        ...
        if service_details.get('result') and service_details['service'].get('enabled'):
            ...
            type = service_name.split('-')[0]

            if type == 'aws':
                # AWS SSM Parameter injection
                aws_accounts = [aws_account for aws_account in credentials if aws_account.get('name').startswith('aws-account-') and aws_account.get('enabled')]
                secret_collection = [s for s in credentials if s.get('enabled') and not s.get('name').startswith('aws-account-')]
                app.logger.debug({"aws_accounts": aws_accounts, "secret_collection": secret_collection })
                for aws_account in aws_accounts:
                    if aws_account.get('name') == 'aws-account-default':
                        # Default behaviour using the inherited AWS role
                        ...
                        try:
                            for entry in secret_collection:
                                ...
                                if 'secret-type' in entry.get('metadata'):
                                    ...
                                    secret_type = 'String'
                                if 'key_id' in entry.get('metadata'):
                                    key_id = entry.get('metedata').get('key_id')
                                secret_pairs = entry.get('credential_pairs')
                                for k, v in secret_pairs.items():
                                    # Inelegant way to handle key id.
                                    if key_id in locals():
                                        response = client.put_parameter(
                                            Name=k,
                                            Description='Managed by Confidant',
                                            Value=v,
                                            Type=secret_type,
                                            KeyId=key_id if key_id,
                                            Tags=[
                                                {
                                                    'Key': 'string',
                                                    'Value': 'string'
